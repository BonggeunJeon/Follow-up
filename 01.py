    ...
    def __init__(self, resize, mean, std):
        self.data_transform = {
            'train': transforms.Compose([
                transforms.RandomResizedCrop(resize, scale=(0.5, 1.0)), # 면적 비율만큼 무작위로 자름
                transforms.RandomHorizontalFlip(), # Horizontal = 좌우반전 
                transforms.ToTensor(), # Pytorch 기본 데이터 형식 - Tensor
                transforms.Normalize(mean, std) # 0 ~ 255 (ToTensor) -> 0 ~ 1 (Normalized) -> -1 ~ 1
            ]),
...
class DogvsCatDataset(Dataset):
    def __init__(self, file_list, transform=None, phase='train'):
        ...
        self.phase = phase # self.phase -> ImageTransform() 에서 정의한 'train'과 'val'
    
    def __len__(self):
        return len(self.file_list)
    
    def __getitem__(self, idx):
        img_path = self.file_list[idx] # Image 데이터 Index 가져오기
        img = Image.open(img_path)
        img_transformed = self.transform(img, self.phase)

        label = img_path.split('/')[-1].split('.')[0] # Lable 값 가져오기
        if label == 'dog':
            label = 1
        elif label == 'cat':
            label = 0
...
def display_image_grid(images_filepaths, predicted_labels=(), cols=5):
    rows = len(image_filepaths)
    figure, ax = plt.subplots(nrow=rows, ncols=cols, figsize=(12, 6))
