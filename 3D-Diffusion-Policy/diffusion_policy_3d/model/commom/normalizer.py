...
from diffusion_policy_3d.model.commom.dict_of_tensor_mixin import DictOfTensorMixin
...
class LinearNormalizer(DictOfTensorMixin):
    ...
    @torch.no_grad()
    def fit(self,
            data: Union[Dict, torch.Tensor, np.ndarray, zarr.Array], # Union하고 Dict이 뭐야??
            last_n_dims=1,
            dtype=torch.float32,
            mode='limits',
            output_max=1.,
    ...
    def __getitem__(self, key: str): # key: str 이게 뭐야??
        return SingleFieldLinearNormalizer(self.params_dict[key])
    
    def __setitem__(self, key: str, value: 'SingleFieldLinearNormalizer'):
        self.params_dict[key] = value.param_dict
    ...
    def _normalize_impl(self, x, forward=True):
        if isinstance(x, dict):
            ...
            if '_default' not in self.params_dict:
                raise RuntimeError("Not initialized") # raise statement가 뭐야? RuntimeError는 어떻게 사용하는거지?
...
class SingleFieldLinearNormalizer(DictOfTensorMixin):
    ...
    @classmethod
    def create_fit(cls, data: Union[torch.Tensor, np.ndarray, zarr.Array], **kwargs): # ** 이게 뭘까?
        obj = cls()         # cls() 가 뭐지?
        obj.fit(data, **kwargs)
        return obj
    ...
    def create_manual(cls, 
        ...
        params_dict = nn.ParameterDict({ # nn.ParameterDict 이게 뭐야??
    ...
    @classmethod
    def create_identity(cls, dtype=torch.float32):  # torch.tensor([1]) 이 뭐고 [0], [-1] 이게 무슨 의미??
...
def test():
    ...
    assert np.allclose(datan.max(), 1.) # np.allclose 가 뭐임?
    ...
    dataun = normalizer.unnormalize(datan)
    for key in data:
        assert torch.allclose(data[key], dataun[key], atol=1e-7)