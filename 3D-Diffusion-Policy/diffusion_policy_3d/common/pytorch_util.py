...
def dict_apply(
        ...
        func: Callable[[torch.Tensor], torch.Tensor] # Callable이 뭐야?
...
def pad_remaining_dims(x, target):
    assert x.shape == target.shape[:len(x.shape)] # assert가 뭐였더라?
...
def dict_apply_reduce(
        ...
        reduce_func: Callable[[torch.Tensor], Dict[str, torch.Tensor]]
        )-> Dict[str, torch.Tensor]:
    result = dict()
    for key in x[0].keys():
